...
class Full_TA_CONTEXT_KI_Advokatpakken_3_9 : public RuleEngineInitialiser {
protected:
    virtual void SetUp() {
    	RuleEngineInitialiser::SetUp();

        KonceptInfo ki {ADVOKATPAKKEN, 5, 0, // UnderkonceptOid:OSV 25-49
        	{
        	  {  1, "true" }, // Solidarisk faellestarif
			  {  6, "true" }, // SEB Firmapensionspulje
        	  { 11, "true" }, // Parameter-Basis
...
TEST_F(Full_TA_CONTEXT_KI_OSV_25_50, Full_TA_POSITIVE_New_InsuranceConditions) {
    KonceptInfo ki {4, 30, 0, // UnderkonceptOid:OSV 25-49
    	{ {  1, "true" }, // Solidarisk faellestarif -> provides access to kGrpIndgaarIDepotsikring_MK
		  {  6, "true" }, // SEB Firmapensionspulje
    	  { 11, "true" }, // Parameter-Basis
		  { 15, "true" }, // FG-Spaend
		  { 29, "2" } // Insurance conditions, version 2 = new insurance conditions
    	} };
    re.initContext(ki, OUTSIDE);
...
TEST_F(Full_TA_CONTEXT_KI_Advokatpakken_3_9, Full_TA_POSITIVE_) {
    KonceptInfo ki {ADVOKATPAKKEN, 5, 0, // UnderkonceptOid:OSV 25-49
    	{ {  1, "true" }, // Solidarisk faellestarif -> provides access to kGrpIndgaarIDepotsikring_MK
		  {  6, "true" }, // SEB Firmapensionspulje
    	  { 11, "true" }, // Parameter-Basis
	...
	ta.setValue(kTAE_RFY, "Ingen");
	ta.setValue(kRetVedInv, 50);
	ta.setValue(kFagspec_mk, true);
	ta.setValue(kFagspecificMaxAgeEnrolled_mk, true);
	...
	ta.setValue(kDoedSkattekode, re.getDefaultValue(kDoedSkattekode)->stringValue());
	ta.setValue(kBoernerente_Reguleringstype, "Gage");
	ta.setValue(kBoerneRentePctMin, (long) 0);
	ta.setValue(kBoerneRentePctMax, (long) 0);
	ta.setValue(kBoerneRenteSoliMax, "Ingen");
	ta.setValue(kBoerneUdloebsalder, (long) 24);
	ta.setValue(kBoerneSumBlMin, (long) 20000);
	ta.setValue(kBoerneSumBlMax, (long) 20000); // +span
	ta.setValue(kKritiskSygdom_i_FG_mk, true); // minus type, min/max/span x 2, skattetype, suppldaekning
	ta.setValue(kKritiskSygReguleringskode, "Gage");
	ta.setValue(kKritiskSygPctMin, (long) 20);
	ta.setValue(kKritiskSygPctMax, (long) 30);
	ta.setValue(kKritiskSygSkattekode, re.getDefaultValue(kKritiskSygSkattekode)->stringValue());
